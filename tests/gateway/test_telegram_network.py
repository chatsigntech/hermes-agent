...
# ═══════════════════════════════════════════════════════════════════════════
# IP-direct (no-SNI) mode helpers — DPI bypass via Host-header + IP base URL
# ═══════════════════════════════════════════════════════════════════════════

class TestIpDirectHelpers:
    def test_base_url_default(self):
        assert tnet.ip_direct_base_url("149.154.167.220") == \
            "https://149.154.167.220/bot"

    def test_base_url_file(self):
        assert tnet.ip_direct_base_url("149.154.167.220", file=True) == \
            "https://149.154.167.220/file/bot"

    def test_ssl_context_disables_verification(self):
        import ssl
        ctx = tnet.build_ip_direct_ssl_context()
        assert ctx.check_hostname is False
        assert ctx.verify_mode == ssl.CERT_NONE

    def test_httpx_kwargs_carry_host_header_and_transport(self):
        kw = tnet.ip_direct_httpx_kwargs()
        assert kw["headers"]["Host"] == "api.telegram.org"
        assert isinstance(kw["transport"], tnet.TelegramNoSNITransport)

    def test_httpx_kwargs_accepts_custom_ctx(self):
        import ssl
        custom = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
        kw = tnet.ip_direct_httpx_kwargs(custom)
        assert kw["transport"]._transport._pool._ssl_context is custom
...
class TestNoSNIConnection:
    @pytest.mark.asyncio
    async def test_omits_server_hostname_when_flag_enabled(self):
        import httpcore

        stream = _FakeTLSStream()
        backend = _FakeNetworkBackend(stream)
        conn = tnet._NoSNIAsyncHTTPConnection(
            origin=httpcore.URL("https://149.154.167.220").origin,
            ssl_context=tnet.build_ip_direct_ssl_context(),
            network_backend=backend,
        )
        request = httpcore.Request(
            method=b"GET",
            url=httpcore.URL("https://149.154.167.220/botTOKEN/getMe"),
            headers=[],
            content=b"",
            extensions={"telegram_no_sni": True, "timeout": {}},
        )

        await conn._connect(request)

        assert stream.server_hostnames == [None]

    @pytest.mark.asyncio
    async def test_preserves_hostname_when_flag_disabled(self):
        import httpcore
        ...
        assert stream.server_hostnames == ["149.154.167.220"]


class TestProbeSniBlock:
    def test_returns_ip_when_sni_fails_but_no_sni_succeeds(self, monkeypatch):
        import socket

        fake_ip = "149.154.166.110"
        monkeypatch.setattr(
            tnet.socket, "getaddrinfo",
            lambda *a, **kw: [(socket.AF_INET, None, None, "", (fake_ip, 443))],
        )
        calls = []

        def fake_attempt(ip, *, server_hostname, timeout):
            calls.append(server_hostname)
            return server_hostname is None

        monkeypatch.setattr(tnet, "_attempt_tls_handshake", fake_attempt)

        assert tnet.probe_sni_block() == fake_ip
        assert calls == ["api.telegram.org", None]

    def test_returns_none_when_handshake_succeeds(self, monkeypatch):
        import socket

        monkeypatch.setattr(
            tnet.socket, "getaddrinfo",
            lambda *a, **kw: [(socket.AF_INET, None, None, "", ("149.154.166.110", 443))],
        )
        monkeypatch.setattr(
            tnet,
        ...
        assert tnet.probe_sni_block() is None
    ...
    def test_returns_none_on_dns_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(tnet.socket, "getaddrinfo", boom)
        assert tnet.probe_sni_block() is None
